        ...
        try {
            ...
            for (let symbol of this.symbols) {
                try {
                    ...
                    const total_btc = new Array(30).fill(0);
                    const total_eth = new Array(30).fill(0);
                    const total_bnb = new Array(30).fill(0);
                    const total_usdt = new Array(30).fill(0);
                    const total_platform = new Array(30).fill(0);
                    ...
                    symbol_results.forEach((kline_result, index) => {
                        if (Array.isArray(kline_result)) {
                            ...
                            if (index === 0) {
                                multipl = btc_usdt;
                                total_platform = total_btc;
                            }
                            else if (index === 1) {
                                multipl = eth_usdt;
                                total_platform = total_eth;
                            }
                            else if (index === 2) {
                                multipl = bnb_usdt;
                                total_platform = total_bnb;
                            }
                            else {
                                multipl = usdt_usdt;
                                total_platform = total_usdt;
                            }

                            kline_result.forEach((datum, i) => {
                                const sum = datum[4] * datum[5] * multipl[index];
                                if (i >= 25) total_05 += sum;
                                if (i >= 20) total_10 += sum;
                                if (i >= 15) total_15 += sum;
                                total_30 += sum;
                                total_platform[i] = sum;
                            });
...

                    {
                        ...
                        console.log(total_bnb)
                        console.log(total_usdt)

                        total_platform.forEach((item, i) => total_platform[i] = total_btc[i] + total_eth[i] + total_bnb[i] + total_usdt[i]);
                    }
            ...
            setTimeout(this.load_symbols_kline.bind(this), 5 * 1000);
        }
    }
